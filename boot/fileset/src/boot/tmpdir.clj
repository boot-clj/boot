      ...
      (visitFile [^Path path attr]
        (with-let [_ fs/continue]
          (let [p (str (.relativize root path))]
            (try (let [h (file/md5 (.toFile path))
...
(defn- apply-mergers!
  ...
  (when-let [merger (some (fn [[re v]] (when (re-find re path) v)) mergers)]
    ...
    (let [out-file (File/createTempFile (file/name merged-file) nil
                                        (file/parent merged-file))]
...
(defn- fatal-conflict?
  ...
  (if (file/directory? dest)
    (let [tree (->> dest file-seq reverse)]
      (or (not (every? #(file/directory? ^File %) tree))
          (doseq [^File f tree] (file/delete f))))
    (not (let [d (file/parent dest)]
           (or (file/directory? d) (.mkdirs d))))))
...
(defrecord TmpFileSet [dirs tree blob scratch]
  ...
  (cp [this src-file dest-tmpfile]
    (let [hash (file/md5 src-file)
          p'   (path dest-tmpfile)
          d'   (dir dest-tmpfile)]
      (assert ((set (map file dirs)) d')
              (format "dest-dir not in dir set (%s)" d'))
      (add-blob! blob (file/path src-file) hash *hard-link*)
